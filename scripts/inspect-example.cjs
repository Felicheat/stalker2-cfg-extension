...
console.log(
  "Total begins by naive scan:",
  naive.pairs.length + naive.unmatchedBegins.length,
  "ends:",
  lines.filter((l) => l.includes("struct.end")).length
);
...
for (const ln of orphanLines) {
  ...
  beforeBlocks.forEach((b) =>
    console.log(`  block ${b.name} start:${b.start + 1} end:${b.end == null ? "null" : b.end + 1}`)
  );
  ...
  console.log(
    "AST finds enclosing block for this line:",
    enclosing
      ? `${enclosing.name} ${enclosing.start + 1}-${enclosing.end == null ? "null" : enclosing.end + 1}`
      : "none"
  );
}
...
console.log(
  "\nNaive unmatched begins count:",
  naive.unmatchedBegins.length,
  "Naive orphan ends count:",
  naive.orphans.length
);
